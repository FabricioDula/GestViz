...
const invoiceTenantLabel = document.getElementById("receipt-tenant");
...
function buildEmbedMapUrlFromInput(input) {
  ...
  // Si ya es un enlace de Google Maps
  ...
  // Texto libre: lo tratamos como búsqueda
  return `https://maps.google.com/maps?q=${encodeURIComponent(
    trimmed
  )}&z=16&output=embed`;
}
...
function renderBuildingGrid() {
  ...
  const pageRows = filteredBuildings.slice(
    startIndex,
  ...
  pageRows.forEach((b) => {
    ...
    // Botón NUEVO PERSONAL (abre popup)
...
if (buildingForm) {
  buildingForm.addEventListener("submit", async (e) => {
    ...
    const internetPrice =
      internetPriceRaw !== "" && !isNaN(Number(internetPriceRaw))
        ? Number(internetPriceRaw)
        : null;
...
// Guardar ubicación de mapa en el inmueble
if (buildingMapForm) {
  buildingMapForm.addEventListener("submit", async (e) => {
    ...
    // Actualizar el estado en memoria si es el inmueble seleccionado
    if (selectedBuildingId === buildingId && selectedBuildingData) {
      selectedBuildingData.mapsUrl = mapsUrl;
      await actualizarResumenEdificio();
    }

    // Refrescar el grid para que se vea el indicador de “Mapa ✓”
    await cargarEdificios();
...
function limpiarEstadoUnidadYInquilinos() {
  ...
  if (invoiceTenantLabel) invoiceTenantLabel.textContent = "Ninguno";
...
function seleccionarEdificio(id, buildingData, opciones = {}) {
  ...
  if (invoiceBuildingLabel) {
    invoiceBuildingLabel.textContent = selectedBuildingName || "Ninguno";
  }
...
async function cargarInquilinos(unitId) {
  ...
  if (snapshot.empty) {
    ...
    tenantList.appendChild(li);
    setTenantFormEnabled(true); // se puede registrar uno nuevo
    activeTenantId = null;
    activeTenantName = null;
    if (invoiceTenantLabel) invoiceTenantLabel.textContent = "Ninguno";
    setInvoiceFormEnabled(false);
  ...
  snapshot.forEach((docSnap) => {
    ...
    const monto =
      data.montoAlquiler != null
        ? Number(data.montoAlquiler).toFixed(2)
        : "—";
    ...
    // CLIC EN LA FILA → usar este inquilino activo para recibos
    main.addEventListener("click", () => {
      if (!data.activo) {
      ...
      if (invoiceTenantLabel) {
        invoiceTenantLabel.textContent = activeTenantName || "Ninguno";
      }
...
if (tenantForm) {
  tenantForm.addEventListener("submit", async (e) => {
    ...
    const montoAlquiler =
      montoRaw !== "" && !isNaN(Number(montoRaw)) ? Number(montoRaw) : null;

    // Desactivar inquilinos activos previos de la unidad
    const qActive = query(
      collection(db, "tenants"),
    ...
    // 🔹 Generar contrato PDF detallado
    ...
    if (invoiceTenantLabel) invoiceTenantLabel.textContent = activeTenantName;
    setInvoiceFormEnabled(true);
...
async function cargarRecibos(unitId) {
  ...
  snapshot.forEach((docSnap) => {
    ...
    const fechaStr =
      data.fechaDia && data.fechaMes && data.fechaAnio
        ? `${String(data.fechaDia).padStart(2, "0")}/${String(
            data.fechaMes
          ).padStart(2, "0")}/${data.fechaAnio}`
        : "Fecha no definida";
    ...
    main.innerHTML = `
      ...
      ${
        notasRecibo
          ? `<div style="font-size:0.75rem; color:#e5e7eb; margin-top:0.2rem;">
               ${notasRecibo}
             </div>`
          : ""
      }
  ...
  pdfBtn.addEventListener("click", () => {
    const infoForPdf = {
      ...
      estado: data.estado,
      notas: data.notas,
      numeroComprobante: id   // 🔹 usamos el ID del documento como N° comprobante
    };
...
function generarPDFContratoAlquiler(info) {
  ...
  const montoTexto =
    info.montoAlquiler != null ? Number(info.montoAlquiler).toFixed(2) : "—";
  ...
  docPdf.text(
    "Ambas partes aceptan las condiciones descritas en este contrato.",
    10,
    120
  );
  ...
  const nombreArchivo =
    (info.tenantNombre
      ? `contrato_${info.tenantNombre.replace(/\s+/g, "_")}`
...
function generarPDFRecibo(info) {
  ...
  const fechaStr =
    info.fechaDia && info.fechaMes && info.fechaAnio
      ? `${String(info.fechaDia).padStart(2, "0")}/${String(
          info.fechaMes
        ).padStart(2, "0")}/${info.fechaAnio}`
      : "Sin fecha";

  const numeroComprobante =
    info.numeroComprobante || info.id || "N/A";
  ...
  const montoTexto =
    info.monto != null ? Number(info.monto).toFixed(2) : "—";
  docPdf.text(`Monto: ${montoTexto}`, 10, 55);
  docPdf.text(
    `Estado del pago: ${(info.estado || "pendiente").toUpperCase()}`,
    10,
    60
  );
...
if (invoiceForm) {
  invoiceForm.addEventListener("submit", async (e) => {
    ...
    if (!selectedBuildingId || !selectedUnitId || !activeTenantId) {
      alert(
        "Selecciona inmueble, unidad e inquilino activo antes de generar un recibo."
      );
      return;
    }

    const dia = invoiceDayInput.value
      ? Number(invoiceDayInput.value)
      : null;
    const mes = invoiceMonthInput.value
      ? Number(invoiceMonthInput.value)
      : null;
    const anio = invoiceYearInput.value
      ? Number(invoiceYearInput.value)
      : null;
    ...
    const monto =
      montoRaw !== "" && !isNaN(Number(montoRaw))
        ? Number(montoRaw)
        : null;
    ...
    // 🔹 Usamos el ID de Firestore como número de comprobante
    generarPDFRecibo({
      ...newInvoice,
      numeroComprobante: docRef.id
    });
